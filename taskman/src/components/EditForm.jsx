...
const EditForm = ({ setIsloading }) => {
  ...
  const handleEditEnquiry = async () => {
    try {
      setIsloading(true);
      const paramsBody = { id: id };
      const response = await userService.GetEnquiry(paramsBody);
      if (response) {
        setIsloading(false);
        reset(response.enquiry);
      }
    } catch (error) {
      setIsloading(false);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await userService.EditEnquiry(jsonBody);
      console.log(response);

      if (!response.error && response.success) {
        setIsloading(false)
        showCustomToast(response.message);
        reset(response.data.enquiry);
      }
    } catch (error) {
      setIsloading(false)
      console.log(error);
