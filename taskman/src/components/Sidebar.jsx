...
const Sidebar = ({ isMobile, drawerOpen, setDrawerOpen }) => {
  ...
  const menuItems = [
    {
      name: "Home",
      path: "/home",
      icon: <AiOutlineHome className="w-5 h-5 mr-3" />,
      admin: false,
    },
    {
      name: "Dashboard",
    ...
    },
    {
      name: "Add Enquiry",
      path: "/add-enquiry",
      icon: <AiOutlinePlus className="w-5 h-5 mr-3" />,
      admin: false,
    },
    {
      name: "Add Employee",
  ...
  return (
    <>
      {isMobile && (
        <div
          className={`fixed flex flex-col justify-between top-0 left-0 w-64 h-full bg-white bg-opacity-100 backdrop-filter backdrop-blur-lg text-gray-900 shadow-lg transition-transform duration-300 ${
        ...
        >
          ...
          <div
            className="bg-red-100 text-red-600 py-2 mb-4 mx-4 px-4 rounded-md flex justify-between items-center"
            onClick={logoutHandler}
          >
            Logout
            <MdLogout className="w-4 h-4 cursor-pointer" />
          </div>
        </div>
      )}
